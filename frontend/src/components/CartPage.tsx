        ...
        {!loading && cart.items.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.items.map((item) => (
                <div key={item.productId._id} className="bg-white rounded-2xl shadow-sm p-6">
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 mb-1">{item.productId.name}</h3>
                    ...
                    <div className="flex items-center space-x-3">
                      <button
                        onClick={async () => {
                          try {
                            await updateQuantity(item.productId._id, Math.max(0, item.quantity - 1));
                          } catch (error) {
                      ...
                      <button
                        onClick={async () => {
                          try {
                            await updateQuantity(item.productId._id, item.quantity + 1);
                          } catch (error) {
                            showError(error instanceof Error ? error.message : 'Failed to update quantity');
                          }
                        }}
                    ...
                    <button
                      ...
                      className="p-3 hover:bg-red-50 text-red-500 rounded-full transition-colors disabled:opacity-50"
                      disabled={loading}
                    >
